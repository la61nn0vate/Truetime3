...
function Register(profile, id_token) {
	...
	gapi.client.accountsApi.get().execute(function(response){
		...
		if(response.error){
			...
			if(status=="ACTIVE")
			{
				gapi.client.accountsApi.update({ 'account_key': key,'name': profile.getName(), 'email': response.email, 'mobileNumber': response.mobileNumber, 'plans':response.plans, 'status':'ACTIVE' }).execute(function (response) {
					...
					}else
					{
						...
						window.location.replace('http://1-dot-truepresent-1.appspot.com/Dashboard/dashboard.html').d;
			...
			{
				gapi.client.accountsApi.update({ 'account_key': key,'name': profile.getName(), 'email': response.email, 'mobileNumber': response.mobileNumber, 'plans':response.plans, 'status':'TRIAL_STARTED' }).execute(function (response) {
					console.log(response);
					if (response.error) {
						console.log(response.error);
						if (response.code == 401) {
...
function choosed_trial() {
	...
	if (v.form()) {
		...
		gapi.client.accountsApi.get().execute(function (response) {
			var key = response.websafeKey;
			if (!response.error) {
				gapi.client.accountsApi.update({ 'status': 'TRIAL_STARTED', 'account_key': key, 'email': response.email, 'mobileNumber': $("#mobileNumber").val() }).execute(function (response) {
...
function add_org_details() {
	...
	if (v.form()) {
		...
		gapi.client.organisationApi.insert({ 'name': $("#org_name").val(), 'location': $("#org_location").val(), 'city': $("#org_city").val(), 'numberOfSites': $("#org_number_of_Sites").val() }).execute(function (response) {
	...
	if (v.form()) {
		gapi.client.organisationApi.getAdminOrganisation().execute(function (response) { //getting org_key from getAdminOrganisation() method of organisationApi
			...
			if (!response.error) {  //Adding Site to the datastore
				gapi.client.siteApi.insert({ 'org_key': org_key, 'site_name': $("#site_name").val(), 'managerEmail': $("#site_manager_email").val(), 'number_of_shifts': $("#site_no_of_shifts option:selected").val(), 'site_location': $("#site_location").val(), 'address': $("#site_address").val(), 'cityOrTown': $("#site_city_town").val() }).execute(function (response) {
					if (!response.error) {
						...
						for (var i = 1; i <= num_of_shift; i++) {
							gapi.client.shiftApi.insert({ 'site_key': site_key, 'name': $("#shift_name" + i).val(), 'time_from': $("#time_from" + i).val(), 'time_to': $("#time_to" + i).val() }).execute(function (response) {
						...
						if (initial > num_of_site) {
							gapi.client.accountsApi.get().execute(function (response) {
								var key = response.websafeKey;
								if (!response.error) {
									gapi.client.accountsApi.update({ 'account_key': key, 'email': response.email, 'mobileNumber': response.mobileNumber, 'plans': response.plans,'status': 'ACTIVE' }).execute(function (response) {
...
function choosed_plan(){
	...
	if (v.form()){
		gapi.client.accountsApi.get().execute(function (response) {
			var key = response.websafeKey;
			if (!response.error) {
				gapi.client.accountsApi.update({ 'account_key': key, 'email': response.email, 'mobileNumber': $("#mobile_number").val(), 'plans':{'name':'plan'+price,'price':price,'validity':'1 month'}, 'status':'REGISTERED' }).execute(function (response) {
					console.log(response);
					if (response.error) {
						console.log(response.error);
						if (response.code == 401) {
...
function payment_doing() {
	...
	if (v.form()) {
		gapi.client.accountsApi.get().execute(function (response) {
			var key = response.websafeKey;
			if (!response.error) {
				gapi.client.accountsApi.update({ 'account_key': key, 'email': response.email, 'mobileNumber': response.mobileNumber, 'plans': response.plans,'status': 'PAYMENT_DONE' }).execute(function (response) {
